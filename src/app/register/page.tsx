      ...
      <main style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '3rem 1rem', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ width: '100%', maxWidth: '440px', position: 'relative' }}>
          <div className="glass-card" style={{ padding: '2rem' }}>
            <h1 style={{ fontFamily: 'var(--font-jakarta)', fontSize: '1.5rem', fontWeight: 800, color: 'var(--text-heading)', marginBottom: '0.25rem' }}>{t('auth.register')}</h1>
            <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem', marginBottom: '1.75rem' }}>{t('auth.registerSubtitle')}</p>
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              <div>
                <label style={{ display: 'block', fontSize: '0.85rem', fontWeight: 600, color: 'var(--text-muted)', marginBottom: '0.375rem' }}>{t('auth.fullName')}</label>
              ...
              <div>
                ...
                {password.length > 0 && (
                  <div style={{ marginTop: '0.5rem', display: 'flex', gap: '0.75rem', fontSize: '0.78rem' }}>
                    <span style={{ color: hasLength ? '#22c55e' : 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: '0.25rem' }}><Check size={12} /> 6+ caractères</span>
                    <span style={{ color: hasUpper ? '#22c55e' : 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: '0.25rem' }}><Check size={12} /> Majuscule</span>
                  </div>
                )}
              </div>
              ...
              <button type="submit" className="btn-primary" disabled={loading} style={{ marginTop: '0.25rem', justifyContent: 'center', opacity: loading ? 0.7 : 1 }}>
                {loading ? t('auth.creating') : <><span>{t('auth.createAccount')}</span><ArrowRight size={16} /></>}
              </button>
            </form>

            <p style={{ textAlign: 'center', marginTop: '1.25rem', fontSize: '0.875rem', color: 'var(--text-muted)' }}>
              {t('auth.alreadyAccount')}{' '}
              <Link href="/login" style={{ color: '#818cf8', fontWeight: 600, textDecoration: 'none' }}>{t('auth.loginLink')}</Link>
            </p>
          </div>
        </div>
      </main>
