...
const ICONS = [
  ...
  { value: 'GraduationCap', label: 'Études' },
  { value: 'Heart', label: 'Santé' },
  { value: 'Smartphone', label: 'Tech' },
  { value: 'Gift', label: 'Cadeau' },
  { value: 'Shield', label: 'Urgence' },
]
...
export default function GoalsPage() {
  ...
  function openAdd() {
    setEditGoal(null)
    setForm({ name: '', targetAmount: '', savedAmount: '0', deadline: '', icon: 'Target', color: '#6366f1' })
    setShowModal(true)
  }
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    try {
      ...
      if (r.ok) { setShowModal(false); load(); showToast(editGoal ? 'Objectif modifié' : 'Objectif créé') }
  ...
  async function handleDelete(id: string) {
    ...
    load(); showToast('Objectif supprimé')
  }

  async function addSaving(goal: SavingGoal) {
    const amount = prompt('Montant à ajouter (DA) :')
    ...
    load(); showToast(`+${Number(amount).toLocaleString()} DA ajoutés`)
  }
  ...
  return (
    <div style={{ paddingBottom: '5rem', maxWidth: '900px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          <h1 style={{ fontFamily: 'var(--font-jakarta)', fontSize: '1.5rem', fontWeight: 800, color: 'white' }}>Objectifs d&apos;épargne</h1>
          <p style={{ color: '#64748b', fontSize: '0.875rem' }}>{goals.length} objectif(s) actif(s)</p>
        </div>
        <button className="btn-primary" onClick={openAdd}><Plus size={18} /> Nouvel objectif</button>
      </div>

      {/* Summary */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1rem', marginBottom: '1.5rem' }}>
        <div className="stat-card">
          <div style={{ fontSize: '0.75rem', color: '#64748b', fontWeight: 600, textTransform: 'uppercase' }}>Objectif total</div>
          <div style={{ fontSize: '1.25rem', fontWeight: 800, color: 'white', marginTop: '0.25rem' }}>{formatCurrency(totalTarget)}</div>
        </div>
        <div className="stat-card">
          <div style={{ fontSize: '0.75rem', color: '#64748b', fontWeight: 600, textTransform: 'uppercase' }}>Épargné</div>
          <div style={{ fontSize: '1.25rem', fontWeight: 800, color: '#22c55e', marginTop: '0.25rem' }}>{formatCurrency(totalSaved)}</div>
        </div>
        <div className="stat-card">
          <div style={{ fontSize: '0.75rem', color: '#64748b', fontWeight: 600, textTransform: 'uppercase' }}>Progression</div>
      ...
      {loading ? (
        ...
        <div className="glass-card" style={{ padding: '3rem', textAlign: 'center', color: '#475569' }}>
          ...
          <p style={{ fontWeight: 600, fontSize: '1rem', color: '#94a3b8' }}>Aucun objectif d&apos;épargne</p>
          <p style={{ fontSize: '0.85rem', marginTop: '0.25rem' }}>Commencez par créer votre premier objectif</p>
          <button className="btn-primary" onClick={openAdd} style={{ marginTop: '1.25rem' }}><Plus size={16} /> Créer un objectif</button>
        </div>
      ) : (
            ...
            return (
              <div key={g.id} className="glass-card" style={{ padding: '1.25rem', position: 'relative', overflow: 'hidden' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1rem' }}>
                  ...
                  <div>
                    <h3 style={{ fontWeight: 700, color: 'white', fontFamily: 'var(--font-jakarta)', fontSize: '0.95rem' }}>{g.name}</h3>
                    {daysLeft !== null && (
                      <span style={{ display: 'flex', alignItems: 'center', gap: '0.25rem', color: daysLeft <= 7 ? '#f59e0b' : '#64748b', fontSize: '0.75rem', marginTop: '0.15rem' }}>
                        <Clock size={11} /> {daysLeft > 0 ? `${daysLeft}j restants` : 'Échéance dépassée'}
                      </span>
                    )}
                  </div>
                </div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '1rem' }}>
                  <div style={{ position: 'relative', width: 64, height: 64, flexShrink: 0 }}>
                    ...
                    <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.8rem', fontWeight: 800, color: g.color || '#6366f1' }}>
                      {pct.toFixed(0)}%
                    </div>
                  </div>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: '0.82rem', color: '#94a3b8', marginBottom: '0.25rem' }}>Épargné</div>
                    <div style={{ fontSize: '1.15rem', fontWeight: 800, color: 'white' }}>{formatCurrency(g.savedAmount)}</div>
                    <div style={{ fontSize: '0.78rem', color: '#64748b', marginTop: '0.15rem' }}>sur {formatCurrency(g.targetAmount)}</div>
                  </div>
                </div>
                ...
                <div style={{ display: 'flex', gap: '0.5rem' }}>
                  <button onClick={() => addSaving(g)} className="btn-primary" style={{ flex: 1, padding: '0.5rem', fontSize: '0.82rem', justifyContent: 'center' }}>
                    <Plus size={14} /> Épargner
                  </button>
                  <button onClick={() => openEdit(g)} style={{ width: 36, height: 36, borderRadius: '8px', background: 'rgba(99,102,241,0.12)', border: '1px solid rgba(99,102,241,0.2)', cursor: 'pointer', color: '#818cf8', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <Pencil size={14} />
                  </button>
                  <button onClick={() => handleDelete(g.id)} style={{ width: 36, height: 36, borderRadius: '8px', background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.2)', cursor: 'pointer', color: '#f87171', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <Trash2 size={14} />
                  </button>
                </div>
              </div>
        ...
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && setShowModal(false)}>
          <div className="modal-box">
            ...
            <form onSubmit={handleSave} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              <div>
                <label style={{ display: 'block', fontSize: '0.82rem', fontWeight: 600, color: '#94a3b8', marginBottom: '0.375rem' }}>Nom de l&apos;objectif</label>
                <input type="text" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} className="input-field" placeholder="Ex: Vacances, Voiture..." required />
              </div>
              <div>
                <label style={{ display: 'block', fontSize: '0.82rem', fontWeight: 600, color: '#94a3b8', marginBottom: '0.375rem' }}>Montant cible (DA)</label>
              ...
              {editGoal && (
                <div>
                  <label style={{ display: 'block', fontSize: '0.82rem', fontWeight: 600, color: '#94a3b8', marginBottom: '0.375rem' }}>Montant épargné (DA)</label>
              ...
              <div>
                <label style={{ display: 'block', fontSize: '0.82rem', fontWeight: 600, color: '#94a3b8', marginBottom: '0.5rem' }}>Couleur</label>
              ...
              <button type="submit" className="btn-primary" disabled={saving} style={{ justifyContent: 'center' }}>
                {saving ? 'Enregistrement...' : editGoal ? 'Modifier' : 'Créer l\'objectif'}
              </button>
            </form>
          </div>
        </div>
